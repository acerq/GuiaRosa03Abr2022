...
const CLIENT_ID = (AMBIENTE == "Desenvolvimento" ? 'f0073a5b-a2e8-4cb8-af4f-cb4c95bf003b' : '');              //### ANTIGO '85012692-b03b-437d-990c-1f0be4a2a377';
const CLIENT_SECRET = (AMBIENTE == "Desenvolvimento" ? '+L9frnKV8EM1AVZYkZCmn+7YJtMOdgU250CIS++9+JU=' : '');  //### ANTIGO 'fwMtZMpiCSnQ45aDSTQpjTb8/xcwO8UJQawRRKQEK1o=';
...
async function doPgtoCC(req, resp) {
  ...
  let browserFingerPrint = id;
	...
	let responseBraspag = await fetch(SERVIDOR_OAUTH2 + '/oauth2/token', requisicao);
  ...
  if(respostaPgto != null && respostaPgto.Payment != null && respostaPgto.Payment.SplitErrors != null) {
    console.log('doPgtoCC SPLIT ERRORS -----------\n');
...
async function doPgtoDebito(req, resp) {
  ...
  let corpo = JSON.stringify(myBody);
	...
	console.log('json doPgtoDébito');
	console.log(respostaPgto);
	console.log('doPgtoDébito LINKS -----------');
	console.log(respostaPgto.Payment.Links);
	console.log('doPgtoDébito SPLIT ERRORS -----------');
	console.log(respostaPgto.Payment.SplitErrors);
